...
def CampaignCount(key): #Grabs all campaigns for given account key, and returns a count of how many campaigns have been run in this console
...
# This ensures the following code is only run when the file is executed directly
if __name__ == "__main__":
    ...
    #List that will contain psu number and the corresponding api.  Could be optimized to only contain the api key, since
    #PSU number is never used in this script.
    Keys = []
    ...
    output = open("PSUAccountSummary.txt", "a") #FKA "PSUTrainingData.txt"
